...
This Neighbourhood Map app displays a google Map with 5 markers. A click on each marker display information associated to that marker. A filter also allows the user to display specific markers

## Folder Structure

After you extract  the files the structure should be as below:
...
NeighborhoodMapReact/
  ...
  src/
    ...
    logo.svg
    MapContainer.js
    registerServiceWorke.rar
********************

## You can download the zip file and extract the contents & perform below steps:

### `rm -rf node_modules && npm install` is preferrrd else `npm install `
...
The page will reloads whenver there an edit in the code also
errors can be seen in the screen.


### `npm run build`

Builds the app for production to the `build` folderIt correctly bundles React in production mode and optimizes the build for the best performance.
Your app is ready to be deployed! on http://localhost:5000/
